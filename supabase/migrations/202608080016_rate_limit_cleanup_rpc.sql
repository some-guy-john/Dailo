create or replace function public.dailo_cleanup_rate_limits(p_before timestamptz)
returns integer
language plpgsql
security definer
...
begin
  ...
  if random() < 0.01 then
    perform public.dailo_cleanup_rate_limits(now() - interval '2 days');
  end if;
...
revoke all on function public.dailo_cleanup_rate_limits(timestamptz) from public, anon, authenticated;
grant execute on function public.dailo_cleanup_rate_limits(timestamptz) to service_role;
